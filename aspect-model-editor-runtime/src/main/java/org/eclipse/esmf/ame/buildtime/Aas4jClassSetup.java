...
import org.eclipse.esmf.ame.substitution.Target_org_eclipse_digitaltwin_aas4j_v3_dataformat_core_internal_util_ReflectionHelper;
...
public class Aas4jClassSetup {
   ...
   public Aas4jClassSetup() {
      ...
      final ScanResult modelScan = new ClassGraph().enableClassInfo().acceptPackagesNonRecursive( MODEL_PACKAGE_NAME ).scan();
   ...
   private Set<Class<?>> scanModelTypes( final ScanResult modelScan ) {
      ...
      typesWithModelTypes = MODEL_TYPE_SUPERCLASSES.stream()
            .flatMap( x -> modelScan.getClassesImplementing( x.getName() ).loadClasses().stream() ).collect( Collectors.toSet() );
   ...
   private Map<Class<?>, Set<Class<?>>> scanSubtypes( final ScanResult modelScan ) {
      return modelScan.getAllInterfaces().stream().filter( this::hasSubclass )
   ...
   private Set<Class<?>> getSubclasses( final ClassInfo clazzInfo ) {
      return new HashSet<>( clazzInfo.getClassesImplementing().directOnly().filter( ClassInfo::isInterface ).loadClasses() );
   }
   ...
   private Map<Class<?>, Class<?>> scanMixins( final ScanResult modelScan, final String packageName ) {
      final ScanResult mixinScan = new ClassGraph().enableClassInfo().acceptPackagesNonRecursive( packageName ).scan();
   ...
   private List<ReflectionHelper.ImplementationInfo> scanDefaultImplementations( final ScanResult modelScan ) {
      final ScanResult defaulImplementationScan = new ClassGraph().enableClassInfo()
            .acceptPackagesNonRecursive( DEFAULT_IMPLEMENTATION_PACKAGE_NAME ).scan();
      final List<ReflectionHelper.ImplementationInfo> defaultImplementations = new ArrayList<>();
      defaulImplementationScan.getAllClasses().filter( x -> x.getSimpleName().startsWith( DEFAULT_IMPLEMENTATION_PREFIX ) ).loadClasses()
            .forEach( x -> {
   ...
   private Set<Class<?>> getInterfacesWithoutDefaultImplementation( final ScanResult modelScan ) {
      return modelScan.getAllInterfaces().loadClasses().stream().filter( x -> !hasDefaultImplementation( x ) )
            .collect( Collectors.toSet() );
   }
