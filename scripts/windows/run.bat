...
if /i "%HostProcessName%"=="explorer.exe" (
  cd /d "%ExeWorkingDir%"
  for %%i in (%~dp0aspect-model-editor-runtime-*.jar) do %~dp0jre\bin\java -Xss4m --enable-native-access=ALL-UNNAMED --sun-misc-unsafe-memory-access=allow -Dpolyglotimpl.DisableMultiReleaseCheck=true -jar "%%i"
  cmd /k
) else (
  for %%i in (%~dp0aspect-model-editor-runtime-*.jar) do %~dp0jre\bin\java -Xss4m --enable-native-access=ALL-UNNAMED --sun-misc-unsafe-memory-access=allow -Dpolyglotimpl.DisableMultiReleaseCheck=true -jar "%%i" %*
)